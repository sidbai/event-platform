...
const { registerNewTeam, registerTeam, setRegistrationStatus, withdrawRegistration } =
...
describe("entering a team that does not exist yet", () => {
  it("creates the team and enters it in one step", async () => {
    // The community-team path: a parent putting a neighbourhood side together
    // has no team on the platform and should not need to build one first.
    const organizer = await makeUser("organizer@test");
    const parent = await makeUser("parent@test");
    const { divisionId } = await makeLeague(organizer);

    signedInUserId = parent;
    const res = await registerNewTeam(
      "wpl-fall",
      {},
      form({ divisionId, teamName: "Wallingford Wanderers" }),
    );
    expect(res.ok).toBe(true);

    const [team] = await db.select().from(teams);
    expect(team.name).toBe("Wallingford Wanderers");
    expect(team.ownerId).toBe(parent);
    expect(team.visibility).toBe("public");

    const [reg] = await db.select().from(eventRegistrations);
    expect(reg.teamId).toBe(team.id);
    expect(reg.status).toBe("requested");
  });

  it("makes the creator the team's owner, so they can manage it after", async () => {
    const organizer = await makeUser("organizer@test");
    const parent = await makeUser("parent@test");
    const { divisionId } = await makeLeague(organizer);

    signedInUserId = parent;
    await registerNewTeam("wpl-fall", {}, form({ divisionId, teamName: "Ballard FC" }));

    const [member] = await db.select().from(teamMembers);
    expect(member.userId).toBe(parent);
  ...
  it("refuses when nobody is signed in", async () => {
    ...
    const res = await registerNewTeam("wpl-fall", {}, form({ divisionId, teamName: "X" }));
    expect(res.error).toMatch(/sign in/i);
  });
  ...
  it("insists on a name", async () => {
    ...
    signedInUserId = parent;
    const res = await registerNewTeam("wpl-fall", {}, form({ divisionId, teamName: " " }));
    expect(res.error).toMatch(/name/i);
    expect(await db.select().from(teams)).toHaveLength(0);
  });

  it("leaves no stray team behind when the division is closed", async () => {
    // The team is created only after the division is known to exist, and the
    // entry is what fails — so a rejected attempt should not litter the teams
    // table with a side nobody plays for.
    const organizer = await makeUser("organizer@test");
    const parent = await makeUser("parent@test");
    const { divisionId } = await makeLeague(organizer);
    await db
    ...
    const res = await registerNewTeam(
      "wpl-fall",
      {},
      form({ divisionId, teamName: "Too Late FC" }),
    );
    expect(res.error).toMatch(/closed/i);
    expect(await db.select().from(eventRegistrations)).toHaveLength(0);
  });

  it("gives two teams of the same name different slugs", async () => {
    const organizer = await makeUser("organizer@test");
    const one = await makeUser("one@test");
    const two = await makeUser("two@test");
    const { divisionId } = await makeLeague(organizer);

    signedInUserId = one;
    await registerNewTeam("wpl-fall", {}, form({ divisionId, teamName: "Green Lake FC" }));
    signedInUserId = two;
    await registerNewTeam("wpl-fall", {}, form({ divisionId, teamName: "Green Lake FC" }));
